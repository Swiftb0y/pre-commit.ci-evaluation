...
def make_mapping(id_mappings_csv):
    csv_reader = csv.DictReader(id_mappings_csv, fieldnames=("source", "destination"))
    return {
        issue_nr_from_url(line["source"]): issue_nr_from_url(line["destination"])
...
def fixup_ids(lp_issues, mapping):
    for issue in lp_issues:
        ...
        if num is None:
            eprint(
                f"issue {issue['id']} has not been " f"imported to github, skipping!"
            )
            continue
...
def main(argv=None):
    ...
    parser.add_argument("--lp_issues_file", type=argparse.FileType("r"), required=True)
