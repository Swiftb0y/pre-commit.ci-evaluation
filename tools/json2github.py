...
class LaunchpadImporter:
    def __init__(self, token, repo, milestonedata, mention):
        ...
        self.gh_labels = {label.name: label for label in self.repo.get_labels()}
        self.gh_milestones = {
    ...
    def format_body(self, issuedata):
        ...
        if issuedata["attachments"]:
            url_strings = [
                f"[{data['title']}]({data['url']})" for data in issuedata["attachments"]
            ]
    ...
    def handle_ratelimit(self, func):
        ...
        while True:
            try:
                ...
                time_to_wait = rate_limit_resettime - datetime.datetime.utcnow()
                ...
                if e.status == 403 and "abuse" in e.data.get("message", ""):
                    ...
                    self.logger.warning(
                        "Triggered secondary rate limit, " "sleeping %d seconds...",
                        abuse_timeout,
                    )
                    sleep_exp_backoff()
                ...
                self.logger.warning(e)
                self.logger.warning("encountered connection error, retrying...")
                sleep_exp_backoff()
            else:
                break
    ...
    def import_label(self, label_name):
        ...
        label = self.handle_ratelimit(lambda: self.repo.create_label(label_name, color))
    ...
    def import_issue(self, issuedata):
        ...
        if issuedata["tags"]:
            labels.extend(
                [self.get_label(tag) for tag in issuedata["tags"] if tag in LABELS]
            )
    ...
    def run_import(self, lp_issues, lp_milestones):
        ...
        for i, issuedata in enumerate(
            sorted(lp_issues.values(), key=lambda x: (x["date_created"], x["id"])),
            1,
        ):
            ...
            if gh_issue_number:
                ...
                if (
                    issuedata.get("gh_comments_imported") == len(issuedata["comments"])
                    and issuedata.get("gh_status_comment_imported", False)
                    and issue_milestone is None
                ):
                    continue

                self.logger.info(f"checking if issue #{gh_issue_number} requires fixup")
                ...
                if issue_milestone is not None:
                    ...
                    if issue.milestone != milestone:
                        ...
                        self.handle_ratelimit(lambda: issue.edit(milestone=milestone))
            else:
            ...
            lp_issues[issuedata["id"]]["gh_comments_imported"] = comments_imported
        ...
        for issuedata in sorted(
            ...
            if duplicate_of is not None:
                duplicate_issue_number = lp_issues[issuedata["id"]]["gh_issue_number"]
                ...
                self.logger.info(
                    f"Marking #{duplicate_issue_number} " f"as duplicate of {original}"
                )
...
def main(argv=None):
    ...
    importer = LaunchpadImporter(args.token, args.repo, lp_milestones, args.mention)
    import_start = datetime.datetime.utcnow()
    try:
        importer.run_import(lp_issues, lp_milestones)
    finally:
        logging.info(f"Import took {datetime.datetime.utcnow() - import_start}")
        if args.output_file:
