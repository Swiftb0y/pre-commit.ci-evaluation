...
def copy_with_gain(output, base_indent_depth, num_channels):
    ...
    for i in range(num_channels):
        ...
        if num_channels > 1:
            ...
            write(
                "%s;" % method_call(copy_with_gain_method_name(num_channels - 1), args),
                depth=2,
            )
        else:
    ...
    terms = ["pSrc%(i)d[i] * gain%(i)d" % {"i": i} for i in range(num_channels)]
    assign = "pDest[i] = "
...
def copy_with_ramping_gain(output, base_indent_depth, num_channels):
    ...
    for i in range(num_channels):
        ...
        write(
            ("const CSAMPLE_GAIN start_gain%(i)d = " "gain%(i)din + gain_delta%(i)d;")
            % {"i": i},
            depth=1,
        )
    ...
    for i in range(num_channels):
        write(
            ("const CSAMPLE_GAIN gain%(i)d = " "start_gain%(i)d + gain_delta%(i)d * i;")
            % {"i": i},
            depth=2,
        )
...
def main(args):
    ...
    output = open(args.sample_autogen_h, "w") if args.sample_autogen_h else sys.stdout
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        epilog=("Example Call:" "./generate_sample_functions.py --sample_autogen_h "),
    )
