...
def is_ammending():
    ppid = os.getppid()
    pppid = subprocess.check_output(("ps", "-p", str(ppid), "-oppid=")).decode().strip()
...
def parse_changelog(content, development_release_date):
    for section in re.split("^## ", content.strip(), flags=re.MULTILINE)[1:]:
        ...
        soup = bs4.BeautifulSoup(markdown.markdown(description_md), "html.parser")
...
def main(argv=None):
    ...
    metainfo_path = os.path.join(rootpath, "res/linux/org.mixxx.Mixxx.metainfo.xml")
