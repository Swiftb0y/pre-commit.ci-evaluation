...
def prepare_deployment(args):
    ...
    metadata = {
        ...
        "file_date": datetime.datetime.fromtimestamp(file_stat.st_ctime).isoformat(),
    ...
    if os.getenv("CI") == "true":
        ...
        metadata.update(
            {
                "git_commit_url": (
                    f"{github_server_url}/{github_repository}/" f"commit/{commit_id}"
                ),
                "build_log_url": (
...
def main(argv=None):
    ...
    artifact_parser.add_argument("file", type=pathlib.Path, help="Local file to deploy")
